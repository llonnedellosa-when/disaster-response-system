...
def report_to_dict(r: IncidentReport):
    return {
        ...
        "severity":         r.severity,
        "status":           r.status.value.capitalize() if hasattr(r.status, "value") else str(r.status).capitalize(),
        "latitude":         r.latitude,
...
async def submit_report(
    ...
    # Save image FIRST, then set image_path
    image_path = None
    if image and image.filename:
        ...
        image_path = save_path.replace("\\", "/")   # normalise on Windows
...
def get_all_reports(
    ...
    if status:      query = query.filter(IncidentReport.status      == status)
    if barangay:    query = query.filter(IncidentReport.barangay    == barangay)
    if report_type: query = query.filter(IncidentReport.report_type == report_type)
    if severity:    query = query.filter(IncidentReport.severity    == severity)

    total   = query.count()
    reports = query.order_by(IncidentReport.created_at.desc()).offset(offset).limit(limit).all()
    return {"status": "success", "total": total, "data": [report_to_dict(r) for r in reports]}


# ── GET REPORTS BY USER (resident app) ───────────────────────────────────────
...
# ── RECENT REPORTS (admin dashboard widget) ───────────────────────────────────
# ⚠️  Must be defined BEFORE /{report_id} or FastAPI parses "admin" as an int
...
# ── REPORT SUMMARY (admin stats) ──────────────────────────────────────────────
# ⚠️  Must be defined BEFORE /{report_id}
@router.get("/admin/summary")
def get_report_summary(db: Session = Depends(get_db)):
    total    = db.query(IncidentReport).count()
    pending  = db.query(IncidentReport).filter(IncidentReport.status == ReportStatus.pending).count()
    reviewed = db.query(IncidentReport).filter(IncidentReport.status == ReportStatus.reviewed).count()
    resolved = db.query(IncidentReport).filter(IncidentReport.status == ReportStatus.resolved).count()
    rate     = round(resolved / total * 100) if total else 0
    return {
        "status": "success",
        "data":   {"total": total, "pending": pending, "reviewed": reviewed, "resolved": resolved, "resolution_rate": rate},
    }
...
def update_report(report_id: int, payload: ReportUpdate, db: Session = Depends(get_db)):
    ...
    if payload.severity    is not None: report.severity    = payload.severity
    if payload.ai_analysis is not None: report.ai_analysis = payload.ai_analysis
    db.commit()
    db.refresh(report)
